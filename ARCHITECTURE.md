...
- PostgreSQL 是关系型事实来源
- Redis 当前保留为可选增强组件，用于缓存与分布式限流；关闭或不可用时主链路仍可运行，但限流会退化为单实例本地窗口，不再承担实时协调预留能力
...
- 通知中心当前采用 `notifications + notification_receipts` 持久化模型，先保证“入箱 + 已读状态 + 未读数 + 列表补拉”闭环；`/api/v1/me/notifications/stream` 仅提供单实例 best-effort SSE 增强，不依赖 WebSocket 或 Redis。
