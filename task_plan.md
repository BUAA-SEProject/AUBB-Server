...
根据 `todo.md` 与最新需求，持续推进作业模块主链路，并把“在线 IDE 第二阶段”后端能力收口到可继续扩展的状态。当前仓库已经完成课程级与班级级作业发布、结构化试卷快照、分题提交、客观题自动评分、人工批改、assignment 级成绩发布、教师侧成绩册第一阶段、真实 go-judge 题目级自动评测、样例试运行与目录树工作区第一阶段。本轮重点是在不扩散到前端编辑器实现的前提下，补齐模板工作区、目录操作、工作区修订历史、历史恢复、自定义标准输入试运行与工作区快照复用，并同步仓库口径。
