...
- 成绩能力已覆盖教师侧开课实例 / 教学班 / 单学生成绩册第一阶段、教师侧课程 / 班级成绩册 CSV 导出与统计报告第一阶段、多作业权重与加权总评第一阶段，以及学生侧开课实例个人成绩册第一阶段。
- 当前最近一次全量验证结果为 `BUILD SUCCESS`，共 `86` 个测试通过。
...
4. 成绩与评测第二阶段：更复杂的总评策略、更完整统计、学生侧导出 / 报表，以及评测结果可复现性与队列稳定化。
