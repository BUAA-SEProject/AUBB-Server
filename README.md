...
AUBB-Server 是 AUBB 平台的后端仓库。当前仓库已经完成平台治理基线，并继续沿模块化单体路线推进课程、assignment、submission、grading、judge、lab/report 与 notification。当前真实代码已覆盖平台配置、学校/学院/课程/班级组织、用户与多身份治理、账号状态、JWT 登录、refresh token 轮换、会话撤销与强制失效、首个学校 / 管理员 bootstrap 初始化闭环、基础审计、站内通知与未读状态，以及课程模板、开课实例、教学班、课程成员、班级功能开关、课程公告、教学资源、课程讨论、作业创建 / 发布 / 关闭、开课实例内题库、结构化试卷快照、文本与附件正式提交、分题答案与客观题自动评分摘要、教师 / 助教人工批改、assignment 级成绩发布、教师与学生成绩册第一阶段、教师侧成绩册 CSV 导出与统计报告第一阶段、学生侧成绩册 CSV 导出第一阶段、成绩册排名与通过率第一阶段、assignment 级批量成绩调整与 CSV 导入导出第一阶段、基于 go-judge 的 assignment 级脚本评测、结构化编程题题目级自动评测、编程题模板工作区、目录树工作区快照、工作区修订历史与恢复、样例试运行历史与日志回放、RabbitMQ 驱动的评测队列第一阶段、共享 MinIO 对象存储接入能力、judge 详细产物对象化存储 phase 2（正式评测报告下载、源码快照归档与产物追踪）、教学班级实验与实验报告 MVP，以及应用 Dockerfile、compose 本地联调入口、GHCR 镜像构建与最小 SSH deploy 流水线基线。

运行时基线为 Spring Boot 4 + Java 25，基础设施目标包括 PostgreSQL、RabbitMQ、MinIO、go-judge、Flyway 与 MyBatis-Plus；Redis 当前保留为可选增强组件，用于缓存和分布式限流，关闭或故障时限流会安全降级到单实例本地窗口。
...
- 课程内容域当前已补齐课程公告、教学资源与课程讨论 MVP，前端可以直接围绕课程主页、班级主页和课程协作区开展页面开发与联调。
...
- 通知中心当前已支持站内通知列表、未读数、单条已读、全部已读，以及作业发布、评测完成、成绩发布、实验发布、实验报告提交 / 评语发布等关键教学事件入箱；当前前端集成基线仍是轮询 + 未读数，`/api/v1/me/notifications/stream` 只作为单实例 best-effort SSE 增强通道，不强依赖 WebSocket。
- 编程题已支持模板工作区、目录树工作区快照、工作区修订历史 / 恢复、基于 `baseRevisionId` 的工作区冲突检测、`AUTO_SAVE` 无变更去噪、IDE 初始化元信息、自定义标准输入试运行、题目级自动评测、`CUSTOM_SCRIPT` 第一阶段、`compileArgs / runArgs` 参数化编译运行、开课实例级 judge environment profiles、题目级 `languageExecutionEnvironments / executionEnvironment` 运行环境快照，以及基于真实 go-judge 验证的 `PYTHON3 / JAVA21 / CPP17 / GO122` 最小执行链路；`JAVA17` 当前仅作为兼容输入保留。
...
- 评测链路已支持 RabbitMQ 队列第一阶段、详细评测报告 API 和测试点级完整日志；学生侧默认脱敏隐藏测试数据，教师侧只有显式具备 `judge.view_hidden` 时才可查看隐藏测试输入输出，编程题基础设施失败时 answer 也会显式进入 `PROGRAMMING_JUDGE_FAILED` 终态，便于区分“失败”与“仍在等待”。
- 成绩能力已覆盖教师侧开课实例 / 教学班 / 单学生成绩册第一阶段、教师侧课程 / 班级成绩册 CSV 导出与统计报告第一阶段、统计报告五档成绩分布与通过率第一阶段、成绩册排名第一阶段、学生侧开课实例个人成绩册与 CSV 导出第一阶段，以及 assignment 级批量成绩调整与 CSV 导入导出第一阶段。V48 已删除成绩申诉、发布快照和 assignment 级成绩权重，当前成绩册回到按结构化作业最新正式提交聚合的简化口径。
...
- Micrometer + Prometheus 当前已补齐最小业务指标基线，运行时通过 `GET /actuator/prometheus` 暴露 judge 队列、judge 执行和成绩发布等关键指标。
...
- Frontend development guide: [docs/frontend-development-guide.md](docs/frontend-development-guide.md)
...
- Prometheus 指标 smoke：`curl -fsS http://localhost:8080/actuator/prometheus | rg 'aubb_(judge|grading)_'`
...
- `bash scripts/api-tests/permission/run_permission_e2e.sh`
...
- [docs/product-specs/permission-system.md](docs/product-specs/permission-system.md)
