...
- 通知中心当前已支持站内通知列表、未读数、单条已读、全部已读，以及作业发布、评测完成、成绩发布、申诉处理完成、实验发布、实验报告提交 / 评语发布等关键教学事件入箱；当前前端集成基线仍是轮询 + 未读数，`/api/v1/me/notifications/stream` 只作为单实例 best-effort SSE 增强通道，不强依赖 WebSocket。
...
- Micrometer + Prometheus 当前已补齐最小业务指标基线，运行时通过 `GET /actuator/prometheus` 暴露 judge 队列、judge 执行、成绩发布和申诉处理等关键指标。
