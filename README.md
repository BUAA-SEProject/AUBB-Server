...
4. 成绩与评测第二阶段：更复杂的总评策略、更完整统计、更丰富报表、更完整申诉流 / 回滚 / SLA，以及评测结果可复现性与队列稳定化。
...
`compose.yaml` 当前只启动 PostgreSQL、RabbitMQ、Redis、MinIO 和 go-judge 等基础设施，不包含应用服务本身；因此 `docker compose up` 不能替代应用运行时的 JWT 密钥注入。
