...
class AuthzLegacyCompatibilityIntegrationTests extends AbstractIntegrationTest {
    ...
    void setUp() {
        jdbcTemplate.execute("""
                TRUNCATE TABLE
                    ...
                    auth_group_members,
                    auth_groups,
                    auth_group_template_permissions,
                    auth_group_templates,
                    auth_permission_defs,
                    platform_configs,
                    users,
                    org_units
    ...
    @Test
    void legacyCollegeAdminShouldStillManageAssignmentsThroughAuthzService() throws Exception {
        String schoolAdminToken = login("school-admin", "Password123");
        String engAdminToken = login("eng-admin", "Password123");
        String teacherToken = login("teacher-main", "Password123");
        ...
        mockMvc.perform(post("/api/v1/teacher/course-offerings/{offeringId}/assignments", offeringId)
                ...
                .andExpect(jsonPath("$.title").value("学院管理员兼容作业"))
