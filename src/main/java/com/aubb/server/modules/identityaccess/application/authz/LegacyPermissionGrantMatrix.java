...
final class LegacyPermissionGrantMatrix {
    ...
    private static final List<PermissionCode> TEACHING_MANAGER_PERMISSIONS = List.of(
    ...
    static List<PermissionCode> forGovernanceRole(GovernanceRole role) {
        return switch (role) {
            case SCHOOL_ADMIN -> merge(SCHOOL_ADMIN_PERMISSIONS, TEACHING_MANAGER_PERMISSIONS);
            case COLLEGE_ADMIN -> merge(COLLEGE_ADMIN_PERMISSIONS, TEACHING_MANAGER_PERMISSIONS);
            case COURSE_ADMIN -> merge(COURSE_ADMIN_PERMISSIONS, TEACHING_MANAGER_PERMISSIONS);
    ...
    static List<PermissionCode> offeringVisibilityForClassMember(CourseMemberRole role) {
        return switch (role) {
            case TA, STUDENT, OBSERVER -> CLASS_MEMBER_OFFERING_VISIBILITY_PERMISSIONS;
            case INSTRUCTOR -> List.of();
        };
    }

    private static List<PermissionCode> merge(List<PermissionCode> first, List<PermissionCode> second) {
