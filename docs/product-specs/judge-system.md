...
- 当前支持结构化编程题样例试运行：
  ...
  - 详细报告与源码快照优先写入对象存储，并分别通过 `detail_report_object_key / source_snapshot_object_key` 建立引用
...
24. 开课实例级 `judge_environment_profiles` 当前作为可复用模板存在；教师在题库题目或 assignment question 中通过 `profileId / profileCode` 引用时，平台会先解析模板，再把结果快照固化进题目配置。
25. 编程题当前支持两种环境选择方式：
...
- `programming_sample_runs`
  ...
  - `programming_language / entry_file_path / artifact_ids_json`：本次样例运行的轻量元数据
  - `source_snapshot_object_key`：样例运行源码快照对象引用
  - `code_text / source_files_json / source_directories_json`：旧代码快照字段，保留兼容回退
  - `stdin_text / expected_stdout`：样例输入输出快照
  - `workspace_revision_id`：可空，引用本次运行所基于的工作区修订
  - `input_mode`：`SAMPLE / CUSTOM`
  - `status`：`RUNNING / SUCCEEDED / FAILED`
  - `verdict`：样例运行的最终判定
  - `stdout_text / stderr_text`：旧完整运行日志字段；对象化后以详细报告根字段为准
  - `result_summary / error_message`：摘要与失败信息
  - `detail_report_object_key`：样例运行详细报告对象引用
  - `detail_report_json`：旧样例运行详细报告字段，保留兼容回退
