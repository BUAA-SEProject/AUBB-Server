...
把结构化作业从“学生可提交、教师可查看”推进到“教师 / 助教可批改、学生按发布状态查看成绩与反馈”，并补齐教师侧与学生侧成绩册、CSV 导出、统计报告、排名、通过率、assignment 级批量调分与 CSV 导入导出第一阶段。当前 grading 模块不接管提交原文，也不执行自动评测；它负责人工评分、反馈写回、assignment 级成绩发布，以及基于现有提交事实的只读成绩册聚合、导出和统计报告。

V48 已简化成绩域：当前实现不再提供 assignment 级成绩权重、加权总评、成绩发布快照批次、成绩申诉 / 复核 API 与对应数据库表。
...
- assignment 级成绩权重与加权总评
- 成绩发布快照批次、冻结、回滚和发布差异比对
- 成绩申诉、复核、仲裁、附件化申诉材料与 SLA 管理
- 更复杂的总评策略与多套权重方案
...
9. 第一次发布会写入 `assignments.grade_published_at / grade_published_by_user_id` 并向学生发送成绩发布通知；重复发布不会重置首发时间。
10. 教师侧成绩册第一阶段默认按“每个学生、每个作业最新一次正式提交”聚合，不提供 `latestOnly=false` 的历史矩阵视图。
...
18. 当前成绩分布固定为五档：`EXCELLENT / GOOD / MEDIUM / PASS / FAIL`；总评分布按学生当前得分率统计，作业分布只统计已提交学生。
19. 成绩册中的 `offeringRank / teachingClassRank` 当前都是只读派生结果，不做持久化存储。
20. 统计报告当前会在总体、按作业和按班级三个层级返回 `passedStudentCount` 与 `passRate`。
21. assignment 级 batch-adjust 与 CSV 导入当前单次最多处理 `100` 条调整项，且所有调整都必须属于同一个 assignment。
...
- `submission_answers`
  ...
  - `grading_status`：`AUTO_GRADED / MANUALLY_GRADED / PROGRAMMING_JUDGED / PROGRAMMING_JUDGE_FAILED / PENDING_MANUAL / PENDING_PROGRAMMING_JUDGE`
  - `feedback_text`：分题反馈
...
- 当前教师侧统计报告已补齐得分率、五档成绩分布和通过率第一阶段，但仍未进入更复杂的总评策略和长期趋势分析。
