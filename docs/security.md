...
- 运行时 `/v3/api-docs` 中的全部 `/api/v1/**` operation 当前都要求映射到明确权限点或固定规则；回归入口是 `AuthzOpenApiPermissionCoverageTests`。
- 越权拒绝当前会写入 `audit_logs.action=AUTHZ_DENIED`，用于真实运行审计与权限矩阵回归。
...
11. 受保护接口新增、改名或调整权限语义时，必须同步更新运行时权限映射回归、真实矩阵脚本 `scripts/realrun/verify_authz_matrix.*`，以及真实 HTTP API 权限回归 `scripts/api-tests/permission/*`。
12. 新增外部认证或令牌刷新机制时，必须同步更新本文件和相关设计文档。
13. 对象存储当前已被 `submission` 附件能力消费；后续业务若继续开放文件上传接口，必须显式定义对象键命名、授权边界和可见性规则。
14. 轮换 JWT 签名密钥会让现有 access token 全部失效；执行轮换前必须同步安排客户端重新登录窗口。
15. `aubb.bootstrap.enabled` 必须默认关闭；仅在新环境初始化窗口临时打开，初始化完成后立即关闭。
