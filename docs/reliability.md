...
13. 评测产物对象化后，列表查询继续只走数据库摘要字段；完整详细报告与样例试运行源码快照走对象引用回放，避免把大对象读取放到热点列表路径。
14. 实验报告当前只保留“每学生每实验一份当前报告”；后续若要引入历史版本，必须新增版本化结构，而不是直接覆盖现有表语义。
15. 通知中心 v1 必须先保证“持久化 + 已读状态 + 未读数 + 列表补拉”闭环，再考虑 WebSocket 推送；实时通道故障不能影响通知入库和已读状态正确性。
16. 涉及热点列表优化时，优先把权限过滤和分页下推到数据库；若组织树或课程成员边界仍需服务层预解析，也应先收敛成有限作用域集合，再交给 SQL 做 count/page，避免全量候选集进入内存。
17. 稳定 API 发生变更时，必须在同一轮提交中同步更新 `docs/stable-api.md`，并至少验证 `/v3/api-docs` 仍可访问且包含当前承诺路径。
