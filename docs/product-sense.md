...
- 教师侧成绩册当前已支持按开课实例 / 教学班导出 CSV，并提供总体摘要、按作业统计、按班级对比、五档成绩分布和通过率的统计报告第一阶段
- 教师侧成绩册与单学生视图当前已补齐开课实例排名和教学班排名第一阶段
- 学生侧当前已支持导出自己在开课实例下的成绩册 CSV，且未发布人工分在导出里会继续按页面可见性规则隐藏
- 学生侧成绩册当前已支持按开课实例查看自己的结构化作业成绩总览
- 成绩域当前已按 V48 简化，保留人工批改、成绩发布、成绩册、导出、统计、排名和批量调分，不再承诺申诉、发布快照或 assignment 权重
- 教师侧当前已支持 assignment 级 batch-adjust，以及基于 CSV 模板的成绩导入 / 导出第一阶段
- 在线 IDE 的后端契约已经具备模板工作区、目录树快照、工作区修订历史、历史恢复和自定义标准输入试运行
- 当前最近一次全量验证结果为 `BUILD SUCCESS`，共 `317` 个后端测试通过
...
4. 成绩与反馈第二阶段：更复杂的总评策略、更深入学习分析，以及学生侧更丰富导出 / 报表扩展位。
